...
def write_and_no_get_response(ser, packet):
    """
    Send the specified packet and don't wait any response.

    Use it only when the register STATUS_RETURN_LEVEL (addr=0x10)
    is set to NO_STATUS_PACKET (=0) or RETURN_ONLY_FOR_READ (=1).
...
def set_id(ser, servo_id, value, verbose = VERBOSE, num_error_attempts = NUM_ERROR_ATTEMPTS):
    """
    Set the ID
    ...
    packet = packets.get_write_id_packet(servo_id, value)
    write_and_get_response_multiple(ser, packet, servo_id, verbose, num_error_attempts)

# def set_register():
#     if reg_addr in reg_1b:
#     elif reg_addr in reg_2b:

def set_reg_1b(ser, servo_id, reg_addr, value, wait_response = True,  verbose = VERBOSE, num_error_attempts = NUM_ERROR_ATTEMPTS):
    """
    set value to register at address reg_addr
    /!\ WARNING: ONLY FOR 1-BYTE LENGTH REGISTER /!\

    :param ser: The ``serial`` object to use.
    :param servo_id: The id of the servo.
    :param reg_addr: The address of the register to write.
    :param value: The data to write.
    :param wait_response: If True, wait a status packet after sending. If False, never wait any response
    :param verbose: If True, status information will be printed. (Default: ``VERBOSE``).
    :param attempts: The number of attempts to make to send the packet when an error is encountered. (Default: ``NUM_ERROR_ATTEMPTS``.)

    If wait_response == True and register STATUS_RETURN_LEVEL (=0x10) is set to RETURN_FOR_ALL_PACKETS (=2)
        :raises: An ``Exception`` if no packet is successfully read after ``attempts`` attempts.
    """

    packet = packets.get_write_packet_1b(servo_id, reg_addr, value)
    if wait_response:
        write_and_get_response_multiple(ser, packet, servo_id, verbose, num_error_attempts)
    else:
        write_and_no_get_response(ser, packet)

def set_reg_2b(ser, servo_id, reg_addr, value, wait_response = True,  verbose = VERBOSE, num_error_attempts = NUM_ERROR_ATTEMPTS):
    """
    set value to register at address reg_addr
    /!\ WARNING: ONLY FOR 2-BYTE LENGTH REGISTER /!\
    ...
    packet = packets.get_write_packet_2b(servo_id, reg_addr, value)
    if wait_response:
        write_and_get_response_multiple(ser, packet, servo_id, verbose, num_error_attempts)
    else:
        write_and_no_get_response(ser, packet)
