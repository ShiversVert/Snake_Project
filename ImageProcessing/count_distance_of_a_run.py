	...
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN:
			if event.key == pygame.K_d:
				print(colored("Définition de la ref", "blue"))
				set_ref = True

			if event.key == pygame.K_f:
				print(colored("Définition de la position finale", "blue"))

				distance = np.sqrt(pow(pos_green[0]-green_ref[0],2) + pow(pos_red[1]-green_ref[1],2))
				print(colored("Distance en pixel :", "blue"))
				print(distance)
				pygame.quit()
